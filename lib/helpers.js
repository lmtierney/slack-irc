...
/**
 * Returns occurances of a current channel's shadow user nicks with their corresponding Slack nicks
 * @return {string}
 */
export function replaceUsernames(slackUsernames, regex, text) {
  return text.replace(regex, (match, slackName) => {
    if (slackUsernames.indexOf(slackName) > -1) {
      return slackName;
    }
    return match;
  });
