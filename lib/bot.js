...
const NICK_SUFFIX = 'slack';
const NICK_REGEX = new RegExp(`^(\\w*)-${NICK_SUFFIX}$`);
...
class Bot {
  ...
  newClient(user, options) {
    const client = new irc.Client(this.server, `${user.name}-${NICK_SUFFIX}`, options);
    client.connect(5, msg => {
      if (msg.rawCommand === '001') {
        this.ircClients[user.id] = client;
        this.attachUserListeners(user);
  ...
  sendToIRC(user, message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        logger.debug(`${user.name}: Sending message to IRC`, channelName, text);
        client.say(ircChannel, text);
      } else if (message.subtype === 'me_message') {
        logger.debug(`${user.name}: Sending action to IRC`, channelName, text);
        client.action(ircChannel, text);
      }
    }
  }
  ...
  sendToSlack(author, channel, text) {
    ...
    if (slackChannelName) {
      ...
      const nickMatch = author.match(NICK_REGEX);
      if (nickMatch && currentChannelUsernames.indexOf(nickMatch[1]) > -1) {
        logger.debug('Ignoring message from shadow user IRC bot.');
        return;
      }
