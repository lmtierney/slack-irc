...
class Bot {
  ...
  attachListeners() {
    ...
    this.slack.rtm.on(RTM_EVENTS.MESSAGE, message => {
      ...
      if (message.type === 'message') {
        ...
        if (message.files || (message.subtype === RTM_MESSAGE_SUBTYPES.FILE_SHARE &&
          /image/.test(message.file.mimetype))) {
          this.sendImageMessage(message, user);
        } else if (message.subtype === RTM_MESSAGE_SUBTYPES.FILE_SHARE &&
          message.file.mode === 'snippet') {
          this.sendSnippetMessage(message, user);
        } else if (message.subtype === RTM_MESSAGE_SUBTYPES.MESSAGE_CHANGED) {
          const text = 'Notice: Message edits are not relayed to IRC.';
          this.slack.web.chat.postEphemeral(message.channel, text, message.message.user,
            { as_user: true });
          ...
          if (CODE_REGEX.test(message.text)) {
            ...
            this.createGist({ content: match[1] }, (err, resp) => {
              if (err == null) {
                final.text = final.text.replace(match[0], resp.html_url);
              }
  ...
  connectNewClient(user) {
    if (!(user.id in this.ircClients)) {
      ...
      userOptions.userName = user.name.replace(/[^0-9a-z]/gi, '');
  ...
  sendMessagesToIRC(user) {
    ...
    for (const channel of _.keys(messageQueue)) {
      ...
      while (messages.length > 0) {
        ...
        if (!message.subtype || message.subtype === RTM_MESSAGE_SUBTYPES.FILE_SHARE) {
