    ...
    this.slack.rtm.on('message', message => {
      ...
      if (message.type === 'message' && message.subtype !== 'bot_message') {
        ...
        this.slack.web.users.info({ user: messageUser }).then(resp => {
          ...
          } else if (message.subtype === 'message_changed' && 'edited' in message.message) {
            const text = 'Notice: Message edits are not relayed to IRC. Users with `APP` next to their name are ' +
                'using IRC.';
      ...
      if (err != null) {
        ...
        this.createGist({
          ...
          if (err2 != null) {
            ...
            final.text = `Added a ${file.pretty_type} snippet: ${resp2.html_url}`;
            if (file.comments_count > 0) {
              final.text += ` with comment: "${file.initial_comment.comment}"`;
            }
          }
          this.queueMessage(user, final);
        });
  ...
  queueMessage(user, message) {
    ...
    this.slack.web.conversations.info({ channel: message.channel }).then(resp => {
      ...
      if (message.thread_ts != null) {
        ...
        const text = 'Notice: Message threads are not relayed to IRC users. Users with `APP` next to their name are ' +
            'using IRC.';
        this.slack.web.chat.postEphemeral({
          channel: message.channel,
          text,
          ...
          thread_ts: message.thread_ts
            ...
            .catch(error => {
              logger.debug(`Error ${error} posting Ephemeral for thread message`);
            });
        return;
      }
      if (!channel) {
