...
const asyncReplace = async (method, str, regex) => {
  ...
  str.replace(regex, (match, id) => {
    promises.push(method(id).then(resp => [id, resp.profile.display_name_normalized]));
  });
...
class Bot {
  ...
  attachListeners() {
    ...
    this.slack.rtm.on('member_joined_channel', message => {
      const text = 'Welcome! This channel has a two-way bridge between the #selenium IRC ' +
        ...
        .then(resp => {
          ...
          for (const key of _.keys(this.ircClients)) {
            if (this.ircClients[key].nick === nick) {
              this.sendToSlack(botUser.profile.display_name_normalized, chan, reply);
  ...
  parseText(text) {
    ...
    return asyncReplace(this.slack.web.conversations.info, str, /<#(C\w+)\|?(\w+)?>/g)
      .then(str2 => asyncReplace(this.slack.web.users.info, str2, /<@(U\w+)\|?(\w+)?>/g))
  ...
  connectNewClient(user) {
    if (!(user.id in this.ircClients)) {
      ...
      userOptions.userName = user.profile.display_name_normalized.replace(/[^0-9a-z]/gi, '');
      if (this.ircClients[user.id] == null) {
        logger.debug(`Connecting a new IRC client for Slack user ${user.profile.display_name_normalized}.`);
  ...
  deleteClient(user, message) {
    const client = this.ircClients[user.id];
    logger.debug(`Deleting client for ${user.profile.display_name_normalized} (${message}).`);
    if (client != null) {
      if (client.conn != null) {
        client.disconnect(message, () => {
          logger.debug(`Bot for Slack user ${user.profile.display_name_normalized} disconnected.`);
        });
      }
  ...
  attachUserListeners(user, client) {
    client.on('error', error => {
      logger.error(`${user.profile.display_name_normalized}: Received error event from IRC`, error);
    ...
    client.on('abort', () => {
      logger.error(`${user.profile.display_name_normalized}: Maximum IRC retry count reached, exiting.`);
  ...
  queueMessage(user, message) {
    ...
    this.slack.web.conversations.info({ channel: message.channel }).then(resp => {
      ...
      const channelName = channel.is_channel ? `#${channel.name_normalized}` : channel.name_normalized;
      const ircChannel = this.channelMapping[channelName];
      logger.debug(`Message queued -- ${name}: ${ircChannel}: ${message.text}`);
      if (ircChannel) {
  ...
  currentChannelUsernames(slackChannel) {
    return slackChannel.members.map(member =>
      this.slack.web.users.info({ user: member })
        .then(resp => resp.profile.display_name_normalized)
        .catch(error => {
